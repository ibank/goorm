...
org.goorm.core.printer.design.preview.prototype = {
	init: function (target, width, height, scale, parent) {
		...
		$(target).find(".canvas").append("<div class='shapes' style='position:absolute; width:" + this.width + "px; height:" + this.height + "px;'></div>"); //This is a grid layer which has grid background image and opacity
			...
			$(this.parent.objects).each(function (i) {
				...
				if($(this)[0].type == 'line') {
					...
					if ($(this)[0].properties.sx) {
						sx = parseInt($(this)[0].properties.sx*self.scale);
					}
					
					if ($(this)[0].properties.sy) {
						sy = parseInt($(this)[0].properties.sy*self.scale);	
					}
	
					if ($(this)[0].properties.ex) {
						ex = parseInt($(this)[0].properties.ex*self.scale);
					}
					...
					if ($(this)[0].properties.sx) {
						sx = parseInt($(this)[0].properties.sx/ratio);
					}
					
					if ($(this)[0].properties.sy) {
						sy = parseInt($(this)[0].properties.sy/ratio);	
					}
	
					if ($(this)[0].properties.ex) {
						ex = parseInt($(this)[0].properties.ex/ratio);
					}

					if ($(this)[0].properties.ey) {					
						ey = parseInt($(this)[0].properties.ey/ratio);
					}
