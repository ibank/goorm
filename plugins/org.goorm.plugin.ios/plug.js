...
org.goorm.plugin.ios.prototype = {
	...
	add_menu_action: function () {
		...
		$("a[action=new_file_ios]").click(function () {
			...
			$(".project_wizard_first_button[project_type=ios]").trigger("click");
			$("#project_new").find(".project_types").scrollTop($(".project_wizard_first_button[project_type=ios]").position().top - 100);
		});
	}
};