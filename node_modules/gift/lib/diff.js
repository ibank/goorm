(function() {
  ...
  module.exports = Diff = (function() {
    ...
    Diff.parse = function(repo, text) {
      var a_blob, a_mode, a_path, b_blob, b_mode, b_path, deleted_file, diff, diff_lines, diffs, lines, m, new_file, renamed_file, sim_index, _ref, _ref2, _ref3, _ref4, _ref5, _ref6;
      lines = text.split("\n");
      diffs = [];
      while (lines.length && lines[0]) {
        _ref = /^diff\s--git\sa\/(.+?)\sb\/(.+)$/.exec(lines.shift()), m = _ref[0], a_path = _ref[1], b_path = _ref[2];
        if (/^old mode/.test(lines[0])) {
          _ref2 = /^old mode (\d+)/.exec(lines.shift()), m = _ref2[0], a_mode = _ref2[1];
          _ref3 = /^new mode (\d+)/.exec(lines.shift()), m = _ref3[0], b_mode = _ref3[1];
        }
        ...
        if (/^new file/.test(lines[0])) {
          _ref4 = /^new file mode (.+)$/.exec(lines.shift()), m = _ref4[0], b_mode = _ref4[1];
          a_mode = null;
          new_file = true;
        } else if (/^deleted file/.test(lines[0])) {
          _ref5 = /^deleted file mode (.+)$/.exec(lines.shift()), m = _ref5[0], a_mode = _ref5[1];
          b_mode = null;
          deleted_file = true;
        ...
        _ref6 = /^index\s([0-9A-Fa-f]+)\.\.([0-9A-Fa-f]+)\s?(.+)?$/.exec(lines.shift()), m = _ref6[0], a_blob = _ref6[1], b_blob = _ref6[2], b_mode = _ref6[3];
        if (b_mode) b_mode = b_mode.trim();
        diff_lines = [];
